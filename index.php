...
$sql = "
    ...
    FROM stalls s
    /* FIX: Removed 'AND s.status = occupied' so Reservations are visible */
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div id="stallModal" style="display:none; position:fixed; z-index:20000; left:0; top:0; width:100%; height:100%; background:rgba(0,0,0,0.5); backdrop-filter:blur(3px);">
            <div style="background:white; margin:5vh auto; width:95%; max-width:1200px; border-radius:12px; overflow:hidden; box-shadow:0 25px 50px -12px rgba(0,0,0,0.25);">
                ...
                <div class="modal-header" style="padding:20px 30px; border-bottom:1px solid #e2e8f0; display:flex; justify-content:space-between; align-items:center;">
                    <h2 id="modalTitle" style="margin:0; font-size:24px; font-weight:800; color:#0f172a;">Stall Info</h2>
                    <span class="close-btn" onclick="closeModal()" style="cursor:pointer; font-size:28px; line-height:1;">&times;</span>
                </div>
                        ...
                        <div id="renterDetails">
                            ...
                            <div id="paymentForm" style="display:none; margin-top:15px; background:#f8fafc; padding:20px; border-radius:8px; border:1px solid #e2e8f0;">
                                <form id="payForm">
                                    ...
                                    <div id="orFieldGroup">
                                        <label>OR Number <span style="color:red">*</span></label>
                                        <input type="text" name="or_no" placeholder="Enter OR #" style="width:100%; padding:10px; margin-bottom:10px; border:1px solid #cbd5e1; border-radius:6px;">
                                    </div>

                                    <div style="display:flex; gap:15px;">
                                        <div style="flex:1;">
                                            <label>Type</label>
                                            <select name="payment_type" id="payType" onchange="toggleMonthInput()">
                                        ...
                                        <div style="flex:1;">
                                            <label>Date</label>
                                            <input type="date" name="date_paid" value="<?php echo date('Y-m-d'); ?>">
                                        </div>
                                    </div>

                                    <div id="rentInfoGroup" style="background:#e0f2fe; padding:10px; border-radius:6px; border:1px solid #bae6fd; margin-bottom:12px;">
                                        <label style="color:#0369a1;">Paying For (Auto)</label>
                                        <input type="month" id="payMonth" name="month_for" readonly style="background:white; cursor:not-allowed;">
                                    </div>
...
<div id="emptyState" style="display:none; text-align:center; padding-top:20px;">
                            ...
                            <div style="display:flex; gap:10px; justify-content:center;">
                                <button id="btnAssign" onclick="toggleTenantForm()" style="padding:12px 20px; background:#3b82f6; color:white; border:none; border-radius:8px; cursor:pointer; font-weight:bold;">
                                    Assign New Tenant
                                </button>
                                <button id="btnReserve" onclick="toggleReserveForm()" style="padding:12px 20px; background:#f59e0b; color:white; border:none; border-radius:8px; cursor:pointer; font-weight:bold;">
                                    Reserve Unit
                                </button>
                            </div>
                        </div>

                        <div id="newTenantForm" style="display:none; margin-top:20px; text-align:left;">
                            <form id="tenantForm" enctype="multipart/form-data">
                                <input type="hidden" id="wizardStallId" name="stall_id">
                                <input type="hidden" id="wizReservationId" name="reservation_id" value="0">

                                <label style="font-size:12px; font-weight:700; color:#475569;">TENANT NAME</label>
                                <input type="text" name="renter_name" required
                                    style="width:100%; padding:10px; margin-bottom:10px; border:1px solid #cbd5e1; border-radius:6px;">

                                <label style="font-size:12px; font-weight:700; color:#475569;">EMAIL ADDRESS (Optional)</label>
                                <input type="email" name="email_address" placeholder="example@email.com"
                                    style="width:100%; padding:10px; margin-bottom:10px; border:1px solid #cbd5e1; border-radius:6px;">

                                <label style="font-size:12px; font-weight:700; color:#475569;">CONTACT NUMBER (11 Digits)</label>
                                <input type="text" name="contact_number" maxlength="11" placeholder="09xxxxxxxxx"
                                ...
                                <label style="font-size:12px; font-weight:700; color:#475569;">START DATE</label>
                                <input type="date" name="start_date" value="<?php echo date('Y-m-d'); ?>"
                                    style="width:100%; padding:10px; margin-bottom:10px; border:1px solid #cbd5e1; border-radius:6px;">

                                <label style="font-size:12px; font-weight:700; color:#475569;">CONTRACT (PDF/IMG)</label>
                                <input type="file" name="contract_file"
                                    style="width:100%; padding:10px; margin-bottom:10px; background:white; border:1px solid #cbd5e1; border-radius:6px;">

                                <label style="font-size:12px; font-weight:700; color:#475569;">PROFILE PHOTO</label>
                                <input type="file" name="profile_image"
                                    style="width:100%; padding:10px; margin-bottom:15px; background:white; border:1px solid #cbd5e1; border-radius:6px;">

                                <button type="button" onclick="submitNewTenant()"
                        ...
                        <div id="reserveForm" style="display:none; margin-top:20px; text-align:left; background:#fffbeb; padding:20px; border-radius:8px; border:1px solid #fcd34d;">
                            <form id="resForm">
                                <input type="hidden" id="resStallId" name="stall_id">

                                <h4 style="margin-top:0; color:#b45309;">Reservation Details</h4>

                                <label style="font-size:12px; font-weight:700; color:#b45309;">APPLICANT NAME</label>
                                <input type="text" name="renter_name" required
                    ...
                    <div class="modal-right" style="flex:1; background:#f8fafc; padding:35px; overflow-y:auto;">
                        <h4 style="margin-top:0; color:#64748b; text-transform:uppercase; letter-spacing:1px;">Payment History</h4>
