...
// 7. Verify Password (Only if sensitive data changed)
if ($sensitive_change) {
    $user_id = $_SESSION['user_id'];
    // FIX: Changed 'password' to 'password_hash' to match your DB
    ...
    if (!in_array($user['role'], ['admin', 'manager'])) {
        sendJson(false, 'Only Admin/Manager can change Dates/Contracts');
    }
    // FIX: Verify against 'password_hash'
    if (!password_verify($admin_pass, $user['password_hash'])) {
        sendJson(false, 'Incorrect Password for Sensitive Changes!');
    }
}

// 8. Update Standard Fields
...
// 9. Update Sensitive Date (If authorized)
